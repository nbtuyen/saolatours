<?php
global $tmpl;
$tmpl -> addStylesheet('default2','blocks/register/assets/css');
...
<div class="popup-login-resgister">
	...
	<div class="tab_login">
		...
		<div class="social_login">
			...
			<a  class="login_gg"  href="#google_login" data-url="<?php echo URL_ROOT ?>index.php?module=users&view=google&raw=1&task=google_login&Itemid=10" data-width="800" data-height="500" data-id="google-login" onclick="openPopupWindow(this);">
		...
		<div class="website_login">
			...
			<form action="<?php echo FSRoute::_("index.php?module=users") ?>" name="login_form" class="login_form" method="post" >
				<div class="row-register cf ">
					<input type="email" name="username" value="" class='txt-login'  placeholder="<?php echo FSText::_('Tên đăng nhập (Email)'); ?>" required />
				</div>
				<div class="row-register cf ">
					<input type="password" name="password" id="password1"  class='txt-login pass' autocomplete="off"  placeholder="<?php echo FSText::_("Mật khẩu") ?>" required/>
				</div>
				<div class="cls save_resetpass">
					...
					<div class="reset_pass">
						<a href="javascript:void(0);" class="forget"><?php echo FSText::_("Quên mật khẩu") ?>?</a>
					</div>
				</div>

				<div class="wrapper-bt-register"> 
					<div class="row-register cf ">
						<input type="submit" name="login" class="lg login" value="<?php echo FSText::_("Đăng nhập") ?>"/>
					</div>
					
					<div class="create">
						Bạn chưa có tài khoản Hải Linh
						<a class="a_register" href="javascript:void(0)" title="<?php echo FSText::_('Tạo tài khoản'); ?>">
	...
	<div class="tab_reset_pass">
		<div class="title">Quên mật khẩu</div>
		<div class="divforget">
			<form action="<?php echo FSRoute::_("index.php?module=users") ?>" name="forget_form" class="forget_form hide" method="post">
				<div class="form-login">
					<div class="text"><span><?php echo FSText::_('Forget password note'); ?></span></div>
					...
					<div class='row-register row-register-capcha'>
						<input type="text" placeholder="<?php echo FSText::_('Mã hiển thị'); ?>"  id="txtCaptcha" value="" name="txtCaptcha"  maxlength="10" size="7" class="capcha txt-login" />
					...
					<div class="sub"> <input type="submit" class='submitbt' name="submitbt" value = "<?php echo FSText::_("Đồng ý");?>"   /></div>
				</div>
				<?php 
				$url = $_SERVER['REQUEST_URI'];
				$return = base64_encode($url);					
	...
	<div class="regis_user">
		<div class="tile">Tạo tài khoản</div>
		<form action="<?php echo FSRoute::_("index.php?module=users&view=users&task=login&Itemid=11") ?>" name="register_form" class="register_form" method="post" onsubmit="javascript: return checkFormsubmit();">
			...
			<div class="form-login">
				<div class="row-register cf ">
					<div class='value fr'>
						<input placeholder="<?php echo FSText::_('Tên đăng nhập (Email)'); ?>" type="text" id="reemail" name="email" class='txtinput fr txt-login' value=""/>
					</div>
				</div>
				<div class="row-register cf ">
					<div class='value fr'>
				...
				<div class="input-check">
					<input type="checkbox" name="check_wi">
					Tôi muốn nhận được ưu đãi và khuyến mãi độc quyền từ Hải Linh
				</div>

				<div class="row-register cf ">
					<div class=" wrapper-bt-register">
						<input type="submit" class='submitbt login' name="submitbt" value = "<?php echo FSText::_('Tạo tài khoản'); ?>"   />
					</div>
				</div>
