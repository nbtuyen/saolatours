			...
			$html .= '<div class="reply_area hide">';
				$html .= '<form action="javascript:void(0);" method="post" name="comment_reply_form_'.$item -> id.'"  id="comment_reply_form_'.$item -> id.'"  class="form_comment cls" onsubmit="javascript: submit_reply('.$item -> id.');return false;">';
					...
					$html .= '<div class="_textarea">';
						$html .= '<textarea id="cmt_content_'.$item -> id.'"  name="content" placeholder="Viết bình luận của bạn..."></textarea>';
					$html .= '</div>';
					$html .='<input type="button" id="btn-comment-mb-rep-'.$item -> id.'" class="btn-comment-mb-rep" onclick="load_btn_rep(this);" value="Gửi bình luận">';
					$html .= '<div class="full-screen-mobile "></div>'; 
					$html .= '<div class="wrap_r cls">   ';    
					if(empty($_COOKIE['user_id'])){
						$html .= '<div class="title-mb">';
						$html .= 'Thông tin người gửi';
						$html .= '<span class="close-md-comment" onclick="off_popup(this);" ><svg height="16px" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
  <g>
...
//							$html .= '</aside>';
							...
							$html .= '<aside class="_right"> ';               
								$html .= '<input class="txt_input" name="name"  id="cmt_name_'.$item -> id.'" type="text" required placeholder="Họ tên (bắt buộc)" maxlength="50" autocomplete="off" value="'.@$_COOKIE['full_name'].'">';
								$html .= '<input class="txt_input" name="email" id="cmt_email_'.$item -> id.'" type="email" required placeholder="Email (bắt buộc)"  value="'. @$_COOKIE['user_email'].'" >';                
...
?>
	...
	<?php if(isset($pagination) && $pagination) echo $pagination->showPagination(3); ?>
<?php }?>
