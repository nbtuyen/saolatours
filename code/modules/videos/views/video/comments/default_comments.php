...
function display_comment_item($item,$childdren,$level,$max_level = 2,$return){
	...
	$html .= '		<input type="text" id="name_'.$item -> id.'" value="Họ tên" name="name" size="40"  onfocus="if(this.value==\'Họ tên\') this.value=\'\'" onblur="if(this.value==\'\') this.value=\'Họ tên\'"/>';
	$html .= '		<input type="text" id="email_'.$item -> id.'" value="Email" name="email" size="40" onfocus="if(this.value==\'Email\') this.value=\'\'" onblur="if(this.value==\'\') this.value=\'Email\'" />';
	$html .= '	</p>';
	...
	$html .= '<input type="hidden" value="contest" name="module" />';
	$html .= '<input type="hidden" value="image" name="view" />';
...
<div class='comments'>
	...
	<form action="#" method="post" name="comment_add_form" id='comment_add_form' class='form_comment'>
		<p class='name_email'>
			<?php $full_name = isset($_SESSION['full_name'])?$_SESSION['full_name']:'Họ tên';?>
			<?php $email = isset($_SESSION['email'])?$_SESSION['email']:'Email';?>
			<input type="text" id="name" value="Họ tên" name="name" size="40"  onfocus="if(this.value=='<?php echo $full_name; ?>') this.value=''" onblur="if(this.value=='') this.value='<?php echo $full_name; ?>'"/>
			<input type="text" id="email" value="Email" name="email" size="40" onfocus="if(this.value=='<?php echo $email; ?>') this.value=''" onblur="if(this.value=='') this.value='<?php echo $email; ?>'" />
		</p>
		<p class='captcha'>
			...
			<a href="javascript:changeCaptcha();"  title="Click vào đây để thay đổi" class="code-view" >
				<img id="imgCaptcha" src="<?php echo URL_ROOT?>libraries/jquery/ajax_captcha/create_image.php" />Click vào đây để thay đổi
			</a>
		</p>	
		...
		<input type="hidden" value="videos" name='module' />
		<input type="hidden" value="video" name='view' />
