...
<?php 
	...
	$module = FSInput::get ( 'rate_module' );
	$view = FSInput::get ( 'rate_view' );
	

	$max_level = 1;
?>
<?php 
function display_rate_item1($item,$childdren,$level,$max_level = 2,$return,$module,$view, $ban_words = array(),$rt_count,$htt){
...
//				$html .= '<a class="favorite" href="javascript: void(0)" >Thích</a>';
		...
		$html .= '<textarea texid = "txt_content_'.$item -> id.'" id="rate_content_'.$item -> id.'" class="rate_content"  name="content" placeholder="Viết bình luận của bạn..."></textarea>';
		$html .= '</div>';
		$html .='<input type="button" class="btn-rate-mb-rep" onclick="load_btn_rep('.$item -> id.');"  value="Gửi bình luận">';
		...
		if(empty($_COOKIE['user_id'])){
			...
			$html .= '<span class="close-md-rate" onclick="off_popup(this);"><svg height="16px" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
			<g>
...
//						}
		...
		$html .= '</form>';
			$html .= '</div>';
		$html .= '</div>';
		// $html .= '<div style="color:red">Cảm ơn bạn đã đánh giá, chúng tôi sẽ phê duyệt đánh giá của bạn sớm nhất !</div>';
	if($level >= $max_level){
		$html .= '</div>';
	}
	if(isset($childdren[$item -> id]) && count($childdren[$item -> id])){
		foreach($childdren[$item -> id] as $c ){
			$html .= display_rate_item1($c,$childdren,$level + 1,$max_level = 2,$return,$module,$view,array(),0,'');
		}
	}
	if($level < $max_level){
		$html .= '</div>';
...
?>
	...
	<?php foreach ($list_parent as $item){ ?>
		<?php $total_rt=$count_rt[$item->id];
				...
				?>
		<?php echo  display_rate_item1($item,$list_children,0,3,$return,$module,$view, $ban_words,$total_rt,$htt);?>
	<?php }?>
	</div>
	<?php if(isset($pagination1) && $pagination1) echo $pagination1->showPagination(3); ?>
<?php }?>
