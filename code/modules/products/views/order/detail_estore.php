<div class="history_order_">
	<div class="form_user_head">
		<div class="form_user_head_l">
			<div class="form_user_head_r">
				<div class="form_user_head_c">
					<span class='bold red'>Th&#244;ng tin gian h&#224;ng</span>
				</div>
			</div>					
		</div>					
	</div>					
	<div class="form_user_footer_body">
		<!-- TABLE 							-->
		<table cellspacing="0" cellpadding="6" border="0" width="100%" class="tabl-info-customer">
			<tbody> 
			  <tr>
				<td width="173px"><b>T&#234;n c&#244;ng ty </b></td>
				<td width="5px">:</td>
				<td><?php echo $estore-> estore_name; ?></td>
			  </tr>
			  <tr>
				<td><b>Gian h&#224;ng address</b></td>
				<td width="5px">:</td>
				<td>
					<?php $link_estores =FSRoute::_('estores.php?module=home&ename='.$estore->estore_url); ?>
					<a href="<?php echo $link_estores; ?>"><?php echo $link_estores; ?></a>
				</td>
			  </tr>
			  <tr>
				<td><b>&#272;&#7883;a ch&#7881;  </b></td>
				<td width="5px">:</td>
				<td><?php echo $estore->address; ?> </td>
			  </tr>
			  <tr>
				<td><b>T&#7881;nh/th&#224;nh ph&#7889; </b></td>
				<td width="5px">:</td>
				<td><?php echo $city_name; ?></td>
			  </tr>
			  <tr>
				<td><b>Qu&#7853;n/huy&#7879;n </b></td>
				<td width="5px">:</td>
				<td><?php echo $district; ?></td>
			  </tr>
			  <tr>
				<td><b>&#272;i&#7879;n tho&#7841;i </b></td>
				<td width="5px">:</td>
				<td><?php echo $estore->telephone; ?></td>
			  </tr>
			 </tbody>
		</table>
		<!-- ENd TABLE 							-->
			
	</div>
</div>