	...
	for($i = 0; $i < count($list_related); $i ++ ){
		...
		<div class="item cls " id="<?php echo $item->alias ?>">
			<div class="question plus">
				<h3 class="title"><a title="<?php echo $item ->question ?>" href="#<?php echo $item->alias ?>"><?php echo $item ->question ?></a></h3>
			</div>
...
<script type="application/ld+json">
{
	...
	"mainEntity":[
				<?php if(!empty($list_related)){
					...
					{
					"@type": "Question", 
					"name": "<?php echo strip_tags($name);?>", 
					"acceptedAnswer": { "@type": 
										"Answer", 
										"text": "<?php echo strip_tags($content);?>" 
									} 
					}
