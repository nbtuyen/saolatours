...
<div class='news_list_body_col_1'>
	...
	for($i = 0; $i < count($relate_tutorial); $i ++ ){
		...
		$link = str_replace('https://','https://www.google.com/amp/s/',$link);
		$link = str_replace('.html','.amp',$link);		
		?>
		<div class="item cls">
			<figure>
				<a href='<?php echo $link;?>' title="<?php echo $item->title;?>">

					<amp-img alt="<?php echo htmlspecialchars($item->title);?>" src="<?php echo URL_ROOT.str_replace('/original/', '/resized/', $item->image);?>"  width="100" height="67"/>
				</a>
			</figure>
