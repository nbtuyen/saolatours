...
<?php 
	class HomeModelsHome extends FSModels
	{
		...
		{
			global $db;
			$query = " SELECT id,name, alias,tags_group,tablename,is_accessories, root_id, is_accessories,list_parents,icon,is_special,name_color
		...
		function get_sub_cats($cat_id){
			return $this -> get_records('published = 1 AND parent_id = '.$cat_id,'fs_products_categories','*',' ordering ASC ',4);
		}
		...
		function get_list($query_body,$limit)
		{
			...
			$query = " SELECT id,name,summary,image, created_time,category_id, category_alias, alias,price,price_old ,discount,published_double,image_double,date_end,date_start,warranty,types,style_types,is_hotdeal,accessories,gift,gift_accessories,name_core,name_display,status_special,name_special,is_hot,is_promotion";
			$query .= $query_body;
			$query .= 'LIMIT '.$limit;
			$db->query($query);
