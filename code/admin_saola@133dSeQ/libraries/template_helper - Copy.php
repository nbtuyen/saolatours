					...
					foreach($col as $item){
						...
						switch($type){
							...
							case 'change_status':
								...
								if($inverse == 1)
									$html_body .= TemplateHelper::changeStatus("cb".($i),$row->$field?$function:"un".$function);
								else 
									$html_body .= TemplateHelper::changeStatus("cb".($i),$row->$field?"un".$function:$function);
								break;
								case 'edit':
...
//				$compare = $value;
	...
	static function dt_edit_selectbox2($title,$name,$value,$default = '',$arry_select = array(),$field_value = 'id', $field_label='name',$size = 1,$multi  = 0,$add_fisrt_option=0,$comment = '',$sub_item = '',$class='chosen-select',$class_col1='col-md-2',$class_col2='col-md-10'){
		$class = $class? $class:'';
		$html = '<div class="form-group">
		<label class="'.$class_col1.' col-xs-12 control-label">'.$title.'</label>
		<div class="'.$class_col2.' col-xs-12">
