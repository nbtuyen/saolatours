<?php
	class Order_statusControllersOrder_status  extends Controllers
	{
	...
	//			print_r($list);die;
			if(empty($list)){
				...
				$excel->obj_php_excel->getActiveSheet()->setCellValue('D1', 'Tên sản phẩm');
				$excel->obj_php_excel->getActiveSheet()->setCellValue('E1', 'Mã sản phẩm');
				$excel->obj_php_excel->getActiveSheet()->setCellValue('F1', 'Thời gian tạo');
				...
				foreach ($list as $item){
					...
					$excel->obj_php_excel->getActiveSheet()->setCellValue('E'.$key, $item->product_code);
