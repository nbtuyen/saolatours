...
function search_videos_related(){
	$('#videos_related_search').click(function(){
		...
		$.get("index2.php?module=landingpages&view=landingpages&task=ajax_get_videos_related&raw=1",{category_id:category_id,keyword:keyword,str_exist:str_exist}, function(html){
...
function set_videos_related(id){
	...
	if(length_children >= max_related ){
		alert('Tối đa chỉ có '+max_related+' video liên quan'	);
		return;
	}
	var title = $('.videos_related_item_'+id).html();                                     
	var html = '<li id="videos_record_related_'+id+'">'+title+'<input type="hidden" name="videos_record_related[]" value="'+id+'" />';
