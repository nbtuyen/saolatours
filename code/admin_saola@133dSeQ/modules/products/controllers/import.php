	...
	function upload_excel($input_name)
	{
		...
		if(	!$excel){
			...
			if($cid){
				...
				if($rs)
				{
					setRedirect("index.php?module=products&view=import&cid=$cid",FSText::_('Có').'<strong> '.$rs.'</strong> '.'bản ghi được cập nhật','suc');
				}
				else 
				{
					echo "1";
					die(1);
					setRedirect("index.php?module=products&view=import&cid=$cid",FSText::_('Không có bản ghi nào được cập nhật'),'alert');
				}	
			}else{
                                        // echo $path."ssss";die;
