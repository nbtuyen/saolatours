<?php 
class ProductsModelsUpdate_product_excel extends FSModels
{
	...
	function import_film_info_new($excel,$path){
		...
		$data = array();
		$check_id = $sheet->getCellByColumnAndRow(0, 1)->getValue();
		if($check_id != 'id'){
			echo "file không đúng định dạng id";
			die;
		}

		$check_msp = $sheet->getCellByColumnAndRow(1, 1)->getValue();
		if($check_msp != 'ma_san_pham'){
			echo "file không đúng định dạng ma_san_pham";
			die;
		}

		$check_name = $sheet->getCellByColumnAndRow(2, 1)->getValue();
		
		if($check_name != 'name'){
			echo "file không đúng định dạng name";
			die;
		}
		
		$check_price_old = $sheet->getCellByColumnAndRow(3, 1)->getValue();

		if($check_price_old != 'price_old'){
			echo "file không đúng định dạng price_old";
			die;
		}

		$check_price = $sheet->getCellByColumnAndRow(4, 1)->getValue();
		if($check_price != 'price'){
			echo "file không đúng định dạng price";
			die;
		}

		$check_cat = $sheet->getCellByColumnAndRow(5, 1)->getValue();
		if($check_cat != 'id_danhmuc'){
			echo "file không đúng định dạng id_danhmuc";
			die;
		}

		$check_cat = $sheet->getCellByColumnAndRow(6, 1)->getValue();
		if($check_cat != 'trang_thai'){
			echo "file không đúng định dạng trang_thai";
			die;
		}
		// die;
		...
		$cats =$this -> get_records('','fs_products_categories','alias,name,root_alias,id,tablename,list_parents,published,alias_wrapper','','','id');
		...
		foreach ($data as $key => $item) {
			...
			$product_exist_code = $this -> get_records('code="' .$item[1].'" AND id != ' . $item[0],'fs_products','id,code');


			if(!empty($product_exist_code)){
				// printr($product_exist_code);
			...
			$product_exist_name = $this -> get_records('name="' .$item[2].'" AND id != ' . $item[0],'fs_products','id,name');

			if(!empty($product_exist_name)){
			...
			$product_exist = $this -> get_record('id="'.$item[0].'"','fs_products','id,code,tablename');

			if(!$product_exist){
				$not_exit_id_txt .= $not_exit_id_int .',';
				$total_not_exit_id++;
				...
				$row['edited_time'] =date('Y-m-d H:i:s');
				$product_ud_id = $this -> _update($row,'fs_products','id = '.$product_exist -> id,1);
				if($product_ud_id){
					$rs_up++;
				}
				if($product_ud_id && $row['tablename'] !='fs_products'){
					$product_exist_tb_extent = $this -> get_record('record_id= "'.$item[0].'"',$row['tablename'],'id,record_id');
					...
					unset($row['gift']);
					unset($row['id']);
					$row['record_id'] = $item[0];
					if(!empty($product_exist_tb_extent)){
						$this -> _update($row,$product_exist->tablename,'record_id = '.$product_exist -> id,1);
					}else{
						$this -> _add($row,$row_tablename,1);
					}
				}
			}
			$not_exit_name_int++;
		...
		$msg =  'Có ' . $rs_up . ' sản phẩm được update';

		if($total_not_exit_id > 0){
			$msg .= ', Có ' .$total_not_exit_id . ' lỗi id không tồn tại dòng ' . $not_exit_id_txt;
		}

		if($total_not_id > 0){
			$msg .= ', Có ' .$total_not_id . ' lỗi không nhập id dòng ' . $not_id_txt;
		}

	
		if($total_not_exit_name > 0){
			$msg .= ', Có ' .$total_not_exit_name . ' lỗi tên sản phẩm đã tồn tại dòng ' . $not_exit_name_txt;
		}

		if($total_not_exit_code > 0){
			$msg .= ', Có ' .$total_not_exit_code . ' lỗi mã sản phẩm đã tồn tại dòng ' . $not_exit_code_txt;
		}

		if($total_not_code > 0){
			$msg .= ', Có ' .$total_not_code . ' lỗi không nhập mã sản phẩm ở các dòng dòng ' . $not_code_txt;
		}
		if($total_not_cat > 0){
			$msg .= ', Có ' .$total_not_cat . ' lỗi không nhập id danh mục ở các dòng dòng ' . $not_cat_txt;
		}

		if($total_not_name > 0){
			$msg .= ', Có ' .$total_not_name . ' lỗi không nhập tên ở các dòng dòng ' . $not_name_txt;
		}
		return $msg;
