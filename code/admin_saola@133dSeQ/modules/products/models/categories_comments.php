<?php 
	class ProductsModelsCategories_comments extends FSModels{
...
//		}
	...
	function ajax_published($published = 1) {
		...
		$rs = $this->_update ( $row, 'fs_products_categories_comments', 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ' );
		echo $rs ? 1 : 0;
		if ($rs) {
	...
	function ajax_del() {
		...
		$rs = $this->_remove ( 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ', 'fs_products_categories_comments' );
		echo $rs ? 1 : 0;
		if ($rs) {
			$this->recal_comments ( $record_id );
		}
	...
	function recal_comments($record_id) {
		$list = $this->get_records ( 'record_id = ' . $record_id, 'fs_products_categories_comments', 'id,published' );
		$total_published = 0;
		...
		$rs = $this->_update ( $row, $this -> table_name, 'id = ' . $record_id . ' ' );
		return $rs;
	}
	function update_unread_for_comments($record_id) {
		$row ['readed'] = 1;
		$this->_update ( $row, 'fs_products_categories_comments', 'record_id = ' . $record_id );
...
//		$row ['created_time'] = $time;
	...
	function save_point() {
		...
		$rs = $this->_update ( $row, FSTable_ad::_ ('fs_products_categories_comments'), ' id = ' . $comment_id );
