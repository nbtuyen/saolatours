...
class ProductsModelsProducts extends FSModels {
	...
	function __construct() {
		...
		$this -> arr_img_paths_video_review = array(array('large',980,510,'cut_image'));
		$this -> arr_img_video_reality = array(array('large',700,700,'resize_image'),array('small',120,120,'cut_image'));
		$this -> arr_image_banner = array(array('large',1800,580,'cut_image'),array('compress',1,1,'cut_image'));
		$this -> arr_image_summary = array(array('large',700,700,'cut_image'));
		$this -> arr_img_paths_strengths = array(array('large',350,350,'cut_image'));
		// $this -> arr_img_paths_other = array(array('large',1000,1000,'cut_image'), array ('small', 120, 120, 'cut_image' ));

		$this -> arr_img_paths_images_reality = array(array('large',1000,1000,'cut_image'), array ('small', 120, 120, 'cut_image' ));
	...
	function save($row = array(), $use_mysql_real_escape_string = 1) {
		...
		if($img_video_reality_name){
			$img_video_reality = $this->upload_image('img_video_reality','_'.time(),2000000,$this -> arr_img_video_reality,$this -> img_folder);
		...
		if($image_banner){
			$image_banner_upload = $this->upload_image('image_banner','_'.time(),2000000,$this -> arr_image_banner,$this -> img_folder);
			if($image_banner_upload){
				$row['image_banner'] = $image_banner_upload;
			}
		}

		//ảnh summary
		$image_summary = $_FILES["image_summary"]["name"];
		if($image_summary){
			$image_summary_upload = $this->upload_image('image_summary','_'.time(),2000000,$this -> arr_image_summary,$this -> img_folder);
			if($image_summary_upload){
				$row['image_summary'] = $image_summary_upload;
			}
		}
	...
	// related products
		...
		if (! $this->remove_strengths ( $id )) {		
		}
		if (! $this->save_exist_strengths ( $id)) {
		}
		...
		if($id){
			...
			if($content){
				$row_fix_text['content'] = html_entity_decode($content);
			}
...
//		}
	...
	function save_new_strengths($record_id){
		global $db;
		for($i = 0; $i < 20; $i ++) {
			$row = array ();
			$row ['title'] = FSInput::get ( 'new_strengths_name_' . $i );
			if (! $row ['title']) {
				continue;
			}
			$row ['record_id'] = $record_id;
			$image_name_menu = $_FILES["new_strengths_image_".$i]["name"];
			if($image_name_menu){
				$image_menu = $this->upload_image('new_strengths_image_'.$i,'_'.time(),2000000,$this -> arr_img_paths_strengths);
				if($image_menu){
					$row['image'] = str_replace('upload_images/','',$image_menu);
				}
			}
			$rs = $this->_add ( $row, 'fs_products_strengths', 1 );
		}
		return true;
	}


	function save_exist_strengths($id) {
		global $db;
		$strengths_exist_total = FSInput::get ('strengths_exist_total');
		$sql_alter = "";
		$arr_sql_alter = array ();
		$rs = 0;
		for ($i = 0; $i < $strengths_exist_total; $i++) {
			$id_strengths_exist = FSInput::get('id_strengths_exist_'.$i);
			$strengths_name = FSInput::get('strengths_name_exist_'.$i);
			if($id_strengths_exist){
				$row = array();
				$row ['title'] = $strengths_name;
				$image_name_menu = $_FILES["strengths_image_exist_".$i]["name"];
				if($image_name_menu){
					$image_menu = $this->upload_image('strengths_image_exist_'.$i,'_'.time(),2000000,$this -> arr_img_paths_strengths);
					if($image_menu){
						$row['image'] = str_replace('upload_images/','',$image_menu);
					}
				}
				$u = $this->_update($row, 'fs_products_strengths', ' id=' . $id_strengths_exist);
				if ($u)
					$rs ++;
			}		
		}
	...
	function remove_strengths($record_id) {
		if (! $record_id)
			return true;
		$other_strengths_remove = FSInput::get('other_strengths',array(),'array');
		$str_other_strengths = implode(',',$other_strengths_remove);
		if ($str_other_strengths) {
			global $db;
			$sql = " DELETE FROM fs_products_strengths
			WHERE record_id = $record_id AND id IN ($str_other_strengths)";
			$db->query ( $sql );
	...
	function get_strengths($tours_id){
		return $this -> get_records('record_id = '.$tours_id,'fs_products_strengths');
	}
	...
	function save_exist_days($id) {
		...
		for ($i = 0; $i < $days_exist_total; $i++) {
			...
			$days_link = FSInput::get('days_link_exist_'.$i);
			$days_title = FSInput::get('days_title_exist_'.$i);
			if($id_days_exist){
				$row = array();
				$row ['link'] = $days_link;
				$row ['title'] = $days_title;
...
//                    echo $days_exist_total;die;
	...
	function save_new_days($record_id){
		...
		for($i = 0; $i < 20; $i ++) {
			$row = array ();
			$row ['link'] = FSInput::get ( 'new_days_link_' . $i );
			if (! $row ['link']) {
				continue;
			}
	...
	function get_data_by_warranty($warranty_id,$record_id){
		...
		function upload_other_images_reality() {
			...
			if ($data [0] == 'add')
				...
				$row ['record_id'] = $data [1];
			$row ['image'] = $this->img_folder . '/original/' . $file_name;
			$rs = $this->_add ( $row, 'fs_' . $this->type . '_images_reality' );
			$row ['id'] = $rs;
			echo json_encode ( $row );
			return true;
		}

		function get_product_images_reality($product_id) {
			if (! $product_id)
				return;
			$query = " SELECT *
			FROM ".FSTable_ad::_('fs_products_images_reality')."
			WHERE record_id = $product_id 
			ORDER BY ordering ASC, image ASC ";
			global $db;
			$sql = $db->query ( $query );
		...
		function uploadAjaxImagesReality() {
			...
			$path = PATH_BASE . $this->img_folder . '/original/';
				...
				//$upload = new Upload ();
			$upload = FSFactory::getClass ( 'FsFiles', '' );
			if (isset($_FILES['filesreality']) && !empty($_FILES['filesreality'])) {
				$no_files = count($_FILES["filesreality"]['name']);
				for ($i = 0; $i < $no_files; $i++) {
					if ($_FILES["filesreality"]["error"][$i] > 0) {
							//echo "Error: " . $_FILES["files"]["error"][$i] . "<br>";
					} else {
						$file_names[$i]  = $fsstring -> stringStandart2($_FILES["filesreality"]["name"][$i]);
						$file_names[$i]  = str_replace('.', '_'.time().'.',$file_names[$i]);
						if (file_exists($path.$file_names[$i])) {
								//echo 'File already exists : '.$path.$file_names[$i]. "<br>";
						} else {
							move_uploaded_file($_FILES["filesreality"]["tmp_name"][$i], $path.$file_names[$i]);
								//echo 'File successfully uploaded :'.$path. $file_names[$i] . "<br>";
						}
			...
			foreach ($file_names as $file_name2 ) {
				...
				if ($data [0] == 'add'){
					$row ['session_id'] = $data [1];
					$max_ordering = $this -> get_max_imagesReality('',$row ['session_id']);
				}else{
					$row ['record_id'] = $data [1];
					$max_ordering = $this -> get_max_imagesReality($row ['record_id']);
				}
				$row ['image'] = $this->img_folder . '/original/' . $file_name;
				$row ['ordering'] = $max_ordering;
				$title = FSInput::get ( 'image_title' );

				$rs = $this->_add ( $row, FSTable_ad::_('fs_products_images_reality') );
			}
			return true;
		...
		function delete_other_image_reality($record_id = 0) {
			...
			if ($listImages) {
				foreach ( $listImages as $item ) {
					$query = '  DELETE FROM '.FSTable_ad::_("fs_products_images_reality").'
					WHERE id = \'' . $item->id . '\'';
					$db->query ( $query );
					$path = PATH_BASE . $item->image;
					@unlink ( $path );
		...
		function sort_other_images_reality() {
			...
			if (isset ( $_POST ["sort"] )) {
				if (is_array ( $_POST ["sort"] )) {
					foreach ( $_POST ["sort"] as $key => $value ) {
						$db->query ( "UPDATE ".FSTable_ad::_('fs_products_images_reality')." SET ordering = $key WHERE id = $value" );
					}
				}
			}
		}

		function get_max_imagesReality($record_id,$session_id = ''){
			$where = '';
			if($record_id)
				$where .= ' record_id = '.$record_id.' ';
			else 
