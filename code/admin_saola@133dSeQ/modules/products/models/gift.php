<?php 
	class ProductsModelsGift extends FSModels
	{
		var $limit;
		var $prefix ;
		function __construct()
		{
			...
			$this -> table_name = 'fs_products_gift';
			// config for save
			$cyear = date('Y');
			$cmonth = date('m');
			$cday = date('d');
			$this -> img_folder = 'images/products/gift/'.$cyear.'/'.$cmonth.'/'.$cday;
			$this -> check_alias = 0;
		...
		function save($row = array(),$use_mysql_real_escape_string = 0) {
			...
			// $price_min =  FSInput::get ('price_min');
			// $price_max =  FSInput::get ('price_max');

			// if($price_min && $price_max && $price_max > 0 AND $price_min < $price_max){
			...
			if (count ( $record_gift )) {
				...
				$row ['products_gift'] = ',' . implode ( ',', $record_gift ) . ',';
			}
			...
			// 	$row2['gift_id'] = $id;
			// 	$row3['list_gift'] = '';
			// 	if(!empty($record_relate) AND !empty($record_gift)){
			// 		$where_record_relate = substr($row ['products_related'], 1, -1);
			// 		$row2['list_gift'] = $row ['products_gift'];
			// 		$rs = $this -> _update($row2, 'fs_products', 'id IN ('.$where_record_relate.')');
			// 	}else{

			// 		if(!empty($record_manu_relate) AND !empty($category_id_wrapper)){
			// 			$where_manufactory_related = substr($row['manufactory_related'], 1, -1);
			// 			$where_category_id_wrapper = substr($row['category_id_wrapper'], 1, -1);

			// 			if(!empty($record_gift)){
			// 				$row2['list_gift'] = $row ['products_gift'];

			// 				$rs_up = $this -> _update($row3, 'fs_products', 'gift_id = '.$id);

			// 				$rs = $this -> _update($row2, 'fs_products', 'manufactory IN ('.$where_manufactory_related.') AND category_id IN ('.$where_category_id_wrapper.')');
			// 			}

			// 		}elseif (!empty($record_manu_relate)) {
			// 			$where_manufactory_related = substr($row['manufactory_related'], 1, -1);

			// 			if(!empty($record_gift)){
			// 				$row2['list_gift'] = $row ['products_gift'];
			// 				$rs_up = $this -> _update($row3, 'fs_products', 'gift_id = '.$id);
			// 				$rs = $this -> _update($row2, 'fs_products', 'manufactory IN ('.$where_manufactory_related.')');
			// 			}

			// 		}elseif (!empty($category_id_wrapper)) {
			// 			$where_category_id_wrapper = substr($row['category_id_wrapper'], 1, -1);
			// 			if(!empty($record_gift)){
			// 				$row2['list_gift'] = $row ['products_gift'];
			// 				$rs_up = $this -> _update($row3, 'fs_products', 'gift_id = '.$id);
			// 				$rs = $this -> _update($row2, 'fs_products', 'category_id IN ('.$where_category_id_wrapper.')');
...
//			}
		...
		function ajax_get_products_gift() {
			...
			$query_body = ' FROM '.FSTable_ad::_('fs_products_list_gift').' ' . $where;
			$ordering = " ORDER BY created_time DESC , id DESC ";
