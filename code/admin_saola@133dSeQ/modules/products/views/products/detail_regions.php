		...
		?>
			<?php if(@$data_by_regions){?>
				<tr>
					...
					<td>
						<select class="select_default_regions select_default_regions_<?php echo $extend_fields[$i]-> id; ?>" data_select="<?php echo $extend_fields[$i]-> id; ?>" name="regions_default_exist_<?php echo $item->id;?>">
							<option value="0">Không</option>
							<option value="1" <?php if(@$data_by_regions-> is_default == 1) echo 'selected'; ?>>Mặc định</option>
						</select>
					</td>
					<td>
			...
			<?php }else{?>
				<tr>
					...
					<td>
						<select class="select_default_regions select_default_regions_<?php echo $extend_fields[$i]-> id; ?>" data_select="<?php echo $extend_fields[$i]-> id; ?>" name="news_regions_default_<?php echo $item->id;?>">
							<option value="0">Không</option>
							<option value="1" <?php if(@$data_by_regions-> is_default == 1) echo 'selected'; ?>>Mặc định</option>
						</select>
...
<script>
	$('.select_default_regions').change(function(){
		var value_select = $(this).val();
		if(value_select == 1) {
			var data_select = $(this).attr('data_select');
			$('.select_default_regions_'+data_select).val(0);
			$(this).val(1);
		}
	})
</script>