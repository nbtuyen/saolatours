<?php 
class Products_soccerModelsCategories extends ModelsCategories
{
	var $limit;
	var $prefix ;
	function __construct()
	{
		...
		$this->arr_img_paths = array (array ('resized', 200, 469, 'cut_image' ), array ('large', 350, 820, 'cut_image' ) );
		...
		function get_tablenames(){
			...
			$list = array_merge( array(0=>(object) array('table_name'=>'fs_products_soccer')),$list);
			return $list;
		}
		function save($row = array(), $use_mysql_real_escape_string = 0){
			...
			if(!$id){
				...
				if($check_name){
					setRedirect('index.php?module=products_soccer&view=categories&task=add',FSText :: _('Tên danh mục đã tồn tại !'),'error');
					return false;
				}
				...
				if($check_name){
					setRedirect('index.php?module=products_soccer&view=categories&task=edit&id='.$id,FSText :: _('Tên danh mục đã tồn tại !'),'error');
					return false;
				}
			}
			...
			if(!empty($name1) AND empty($name2)){
				if(!$id){
					setRedirect('index.php?module=products_soccer&view=categories&task=add',FSText :: _('Phải nhập cả tên đoạn 1 và tên đoạn 2 !'),'error');
					return false;
				}else{
					setRedirect('index.php?module=products_soccer&view=categories&task=edit&id='.$id,FSText :: _('Phải nhập cả tên đoạn 1 và tên đoạn 2 !'),'error');
						return false;
				}
			}
			...
			if(!empty($alias1) AND empty($alias2)){
				if(!$id){
					setRedirect('index.php?module=products_soccer&view=categories&task=add',FSText :: _('Phải nhập cả tên hiệu 1 và tên hiệu 2 !'),'error');
					return false;
				}else{
					setRedirect('index.php?module=products_soccer&view=categories&task=edit&id='.$id,FSText :: _('Phải nhập cả tên hiệu 1 và tên hiệu 2 !'),'error');
						return false;
				}
			}
			...
			if($id){
				...
				if($data_id){
					...
					if($tablename != $table_id_name ){
						...
						if($count_data > 0){
							setRedirect('index.php?module=products_soccer&view=categories&task=edit&id='.$id,FSText :: _('Đã tồn tại sản phẩm trong danh mục nên ko chuyển được bảng !'),'error');
							return false;
						}
					}
				}
		...
		function update_products_compatable($category_id){
			...
			if(empty( $arr )){
				$this -> _remove('category_id = '.$category_id ,'fs_products_soccer_categories_kilogam');
				return;
			} 
			...
			$this -> _remove(' manufactory_id NOT IN (0'.$str_prices_is.'0) AND category_id = '.$category_id ,'fs_products_soccer_categories_kilogam');
			...
			foreach($arr_prices as $item){
				...
				$check_ex =  $this -> get_records('manufactory_id ='.$item-> id.' AND category_id = '.$category_id ,'fs_products_soccer_categories_kilogam','*');

				if(!empty($check_ex)){	
					$this -> _update($row, 'fs_products_soccer_categories_kilogam','category_id ='.$category_id.' AND manufactory_id = '.$item -> id);
				}else{
					$this -> _add($row, 'fs_products_soccer_categories_kilogam');
				}
			}
			return true;
		}
		...
		function update_seo_manu($category_id){
			...
			if(empty( $arr )){
				$this -> _remove('category_id = '.$category_id ,'fs_products_soccer_categories_seo_manufactory');
				return;
			} 
			...
			$this -> _remove(' manufactory_id NOT IN (0'.$str_prices_is.'0) AND category_id = '.$category_id ,'fs_products_soccer_categories_seo_manufactory');
			...
			foreach($arr_prices as $item){
				...
				$check_ex =  $this -> get_records('manufactory_id ='.$item-> id.' AND category_id = '.$category_id ,'fs_products_soccer_categories_seo_manufactory','*');

				if(!empty($check_ex)){	
					$this -> _update($row, 'fs_products_soccer_categories_seo_manufactory','category_id ='.$category_id.' AND manufactory_id = '.$item -> id);
				}else{
					$this -> _add($row, 'fs_products_soccer_categories_seo_manufactory');
				}
			}
			return true;
		}
		...
		function get_products_categories_kilogam($cat_id){
		 	if(!$cat_id)
		 		return;
		 	$query   = " SELECT *
		 	FROM fs_products_soccer_categories_kilogam
		...
		function get_seo_manu($cat_id){
		 	if(!$cat_id)
		 		return;
		 	$query   = " SELECT *
		 	FROM fs_products_soccer_categories_seo_manufactory
		 	WHERE category_id = '$cat_id'
		 	ORDER BY id ASC
		 	";
		 	global $db;
...
function ajax_get_aq_related(){
	...
	function published($value)
	{
		...
		function ajax_move_category(){
			...
			$cat_id_move = $this->get_record('id = '. $id_move,'fs_products_soccer_categories');
			$cat_id_move_to = $this->get_record('id = '. $id_move_to,'fs_products_soccer_categories');
