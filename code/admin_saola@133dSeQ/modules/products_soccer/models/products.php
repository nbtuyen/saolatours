<?php
class Products_soccerModelsProducts extends FSModels {
	var $limit;
	var $prefix;
	var $image_watermark;
	function __construct() {
		...
		// $this -> arr_img_paths = array(array('resized',300,300,'resize_image'),array('small',120,120,'cut_image'),array('large',800,800,'resize_image'));
		// $this -> arr_img_paths_other = array(array('resized',300,300,'resize_image'),array('small',120,120,'cut_image'),array('large',800,800,'resize_image'));
		...
		$this -> arr_img_paths_video_review = array(array('large',800,400,'cut_image'));
		$this -> arr_img_paths_image_summary = array(array('large',800,400,'cut_image'));
		$this -> arr_img_video_reality = array(array('large',700,700,'resize_image'),array('small',120,120,'cut_image'));
		$this->check_alias = 1;
	...
	function setQuery() {
		...
		if (isset ( $_SESSION [$this->prefix . 'filter0'] )) {
			...
			if ($filter) {
				$where .= ' AND a.category_id_wrapper_extra like   "%,' . $filter . ',%" ';
			}
		}
		// lọc loại sản phẩm
		...
		if (isset ( $_SESSION [$this->prefix . 'filter8'] )) {
			$filter = $_SESSION [$this->prefix . 'filter8'];
			if ($filter) {
				if($filter == 1)
		...
		// SP km
		if (isset ( $_SESSION [$this->prefix . 'filter9'] )) {
			$filter = $_SESSION [$this->prefix . 'filter9'];
			if ($filter) {
				if($filter == 1)
					$where .= ' AND a.is_promotion = 1';
				else 
					$where .= ' AND a.is_promotion != 1';
			}
		...
		if (isset ( $_SESSION [$this->prefix . 'keysearch'] )) {
			if ($_SESSION [$this->prefix . 'keysearch']) {
				...
				$keysearch = stripslashes($keysearch);
	...
	function get_categories_products_tree() {
		$where ='';
		if (isset ( $_SESSION [$this->prefix . 'category_keysearch'] )) {
			if ($_SESSION [$this->prefix . 'category_keysearch']) {
				$keysearch = $_SESSION [$this->prefix . 'category_keysearch'];
		...
		global $db;
		$sql = " SELECT id, name,list_parents, parent_id AS parent_id 
		FROM fs_products_categories ORDER BY ordering ASC";
		$db->query ( $sql );
		$categories = $db->getObjectList ();
	...
	function get_manufactories() {
		global $db;
		$query = " SELECT id, name 
		FROM fs_manufactories ORDER BY ordering ASC";
		global $db;
		$sql = $db->query ( $query );
		$result = $db->getObjectList ();
		return $result;
	...
	function save($row = array(), $use_mysql_real_escape_string = 1) {
		...
		if(!$id){
			$link = 'index.php?module=products_soccer&view=products&task=add&cid='.$category_id;
		}else{
			$link = 'index.php?module=products_soccer&view=products&task=edit&id='.$id;
		}
		...
		$category_id_arr = FSInput::get ( 'category_id_wrapper_extra',array (), 'array');

		$category_id = array();

		foreach ($category_id_arr as $cat_item) {
			$cat_item_check = $this->get_record('id = ' . $cat_item,'fs_products_soccer_categories');
			if($cat_item_check -> tablename == $cat->tablename ||  $cat_item_check -> tablename == 'fs_products'){
		...
		{ 
			$item = $category_id[$i];
			$alias = $this -> get_record_by_id($item,'fs_products_soccer_categories','alias,list_parents');
		...
		{ 
			$item = $array_id_emp[$j];
			$alias = $this -> get_record_by_id($item,'fs_products_soccer_categories','alias');
		...
		foreach ($array_merge as $val) {
			$get_alias = $this->get_record('id= ' .$val ,'fs_products_soccer_categories');
			if($get_alias){
		...
		$row ['category_id_wrapper_extra'] = ','.$str_show_count.',';
		$row ['category_alias_wrapper_extra'] = ','. $str_show_count_alias;
		...
		if(FSInput::get ('price')){
			$row ['price_old'] = $price_old;
			$row ['discount'] = (($price_old - $price) / $price_old) * 100;
			$row ['price'] = $price;	
		}else{
		...
		//img_video_reality
		$img_video_reality_name = $_FILES["img_video_reality"]["name"];
		if($img_video_reality_name){
			$img_video_reality = $this->upload_image('img_video_reality','_'.time(),2000000,$this -> arr_img_video_reality,$this -> img_folder2);
			if($img_video_reality){
				$row['img_video_reality'] = $img_video_reality;
			}
		}
		...
		// remove color
			if (! $this->remove_color ( $id,$row ['tablename'] )) {
			}
		// edit color
			if (! $this->save_exist_color ( $id, $row ['tablename'] )) {
			//				return false;
		}
		...
		//tiện ích
		if (! $this->remove_utilities( $id )) {
		}	
		if (! $this->save_exist_utilities ( $id )) {
		}
		if (! $this->save_new_utilities ( $id )) {
		}



		//khung giờ
		if (! $this->remove_range_times($id)) {
		}	
		if (! $this->save_exist_range_times($id)) {
		}
		if (! $this->save_new_range_times($id)) {
		}
		...
		if (! $this->remove_image_summary ( $id )) {		
		}
		if (! $this->save_exist_image_summary ( $id)) {
		}
		...
		// sân bóng liên quan
		$record_soccer_relate = FSInput::get('products_soccer_record_related',array(),'array');
		$row['products_soccer_related'] ='';
		if(count($record_soccer_relate)){
			$record_soccer_relate = array_unique($record_soccer_relate);
			$row['products_soccer_related'] = ','.implode(',', $record_soccer_relate).',';	
		}
		...
		// related combo
		$record_combo = FSInput::get('products_record_combo',array(),'array');
		$row['combo_products_id'] ='';
		if(count($record_combo)){
			$record_combo = array_unique($record_combo);
			$row['combo_products_id'] = ','.implode(',', $record_combo).',';
			$row['combo_count']	 = count($record_combo);
		}
	...
	// related products
		...
		if($file_upload){
			$path_original = '../images/products/download_file/';
			if($id){
				$img_paths = array();
			...
			if(!$file_upload_name)
				return false;
			$row['file_upload'] = 'images/products/download_file/'.$file_upload_name;
		}
		...
		if($check_save_new) {
			...
			$row_img['session_id'] = '';
			$this-> _update($row_img,'fs_products_soccer_images','session_id = "'.session_id().'"');
		}
		...
		if($id){
			...
			if($description){
				$row_fix_text['description'] = html_entity_decode($description);
			}

			$this->_update2( $row_fix_text, 'fs_products', 'id = ' .$id );
	

			$this->save_new_days( $id );
			$this->save_new_image_summary( $id );
			...
			$get_image_color = $this->get_record('record_id=0 AND session_id ="'.$data_session.'"','fs_products_soccer_images');
			$row_id['record_id']=$id;
			$row_id['session_id']='';
			if(!empty($get_image_color)){
				$this->_update ( $row_id, 'fs_products_soccer_images', 'session_id = "'.$data_session.'"' );
			}
	...
	function remove_days($record_id) {
		...
		if ($str_other_days) {
			global $db;
    				...
    				// remove in database
			$sql = " DELETE FROM fs_products_soccer_video_review
...
//                    echo $days_exist_total;die;
				$u = $this->_update($row, 'fs_products_soccer_video_review', ' id=' . $id_days_exist);
				if ($u)
					$rs ++;
			}		
		}
	...
	function save_new_days($record_id) {
		...
		for($i = 0; $i < 20; $i ++) {
			...
			$rs = $this->_add ( $row, 'fs_products_soccer_video_review', 1 );
		}
		return true;
	}

	function get_days($tours_id){
		return $this -> get_records('record_id = '.$tours_id,'fs_products_soccer_video_review');
	}
	///end video

	//image summary
	function save_new_image_summary($record_id) {
		global $db;
		for($i = 0; $i < 20; $i ++) {
			$row = array ();
			$row ['summary'] = FSInput::get ( 'new_image_summary_content_' . $i );
			if (! $row ['summary']){
				continue;
			}
			$row ['record_id'] = $record_id;
			...
			$image_name_menu = $_FILES["new_image_summary_image_".$i]["name"];
			if($image_name_menu){
				$image_menu = $this->upload_image('new_image_summary_image_'.$i,'_'.time(),2000000,$this -> arr_img_paths_image_summary);
				if($image_menu){
					$row['image'] = str_replace('upload_images/','',$image_menu);
				}
			}

			$rs = $this->_add ( $row, 'fs_products_soccer_image_summary', 1 );
		}
		return true;
	}

	function save_exist_image_summary($id) {

		global $db;
		$days_exist_total = FSInput::get ( 'image_summary_exist_total' );
		$sql_alter = "";
		$arr_sql_alter = array ();
		$rs = 0;
		for ($i = 0; $i < $days_exist_total; $i++) {
			$id_days_exist = FSInput::get('id_image_summary_exist_'.$i);
			$days_name = FSInput::get('image_summary_content_exist_'.$i);
			if($id_days_exist){
				$row = array();
				$row ['summary'] = $days_name;
				$image_name_menu = $_FILES["image_summary_image_exist_".$i]["name"];
				if($image_name_menu){
					$image_menu = $this->upload_image('image_summary_image_exist_'.$i,'_'.time(),2000000,$this -> arr_img_paths_image_summary);
					if($image_menu){
						$row['image'] = str_replace('upload_images/','',$image_menu);
					}
				}
				$u = $this->_update($row, 'fs_products_soccer_image_summary', ' id=' . $id_days_exist);
				if ($u)
					$rs ++;
			}		
		}
	...
	function remove_image_summary($record_id) {
		if (! $record_id)
			return true;
		$other_days_remove = FSInput::get('image_summary',array(),'array');
		$str_other_days = implode(',',$other_days_remove);
		if ($str_other_days) {
			global $db;
			$sql = " DELETE FROM fs_products_soccer_image_summary
			WHERE record_id = $record_id AND id IN ($str_other_days)";
			$db->query ( $sql );
			$rows = $db->affected_rows ();
...
//					return false;
	...
	function get_product_images($product_id) {
		if (! $product_id)
			return;
		$query = " SELECT *
		FROM fs_products_soccer_images
	...
	//				$this -> _update($row, 'fs_products_soccer_categories',' WHERE id = '.$cat_id.' ');
	//			}
	//		}
	...
	function get_products_soccer_related($product_related){
		if(!$product_related)
			return;
		$query   = " SELECT id, name 
	...
	function get_products_combo($product_related){
		if(!$product_related)
			return;
		$query   = " SELECT id, name 
		FROM fs_products
	...
	function ajax_get_products_soccer_related(){
		$news_id = FSInput::get('product_id',0,'int');
		$category_id = FSInput::get('category_id',0,'int');
		$keyword = FSInput::get('keyword');
		$where = ' WHERE published = 1 AND is_trash = 0 ';
		...
		$query_body = ' FROM fs_products_soccer '.$where;
		$ordering = " ORDER BY created_time DESC , id DESC ";
		$query = ' SELECT id,category_id,name,category_name '.$query_body.$ordering.' LIMIT 40 ';
		global $db;
		$sql = $db->query($query);
...
function ajax_get_news_related(){
	...
	function save_exist_color($record_id, $tablename) {
		...
		foreach ($other_color_exit as $item){
			...
			//$row ['image'] =$color->image;
			$row ['price'] = FSInput::get ( 'color_price_exist_' . $item );
			$price_old = FSInput::get ( 'color_price_old_exist_' . $item );
			if(!$price_old){
				$price_old = $row ['price'];
			}
			$row ['price_old'] = $price_old;
			...
			$check_image_default = $this->get_record('record_id = ' . $record_id . ' AND color_id = ' . $item,'fs_products_soccer_images');

			if(!empty($check_image_default)){
				$row2['is_default'] = $row ['is_default'];
				$rs2 = $this->_update ( $row2, 'fs_products_soccer_images', ' id=' . $check_image_default-> id );
			}
		...
		$this-> update_record_color($record_id,$tablename);
	...
	function save_new_color($record_id) {
		...
		foreach($other_color as $item){
			...
			$price_old = FSInput::get ( 'new_color_price_old_' . $item );
			if(!$price_old){
				$price_old = $row ['price'];
			}
			$row ['price_old'] = FSInput::get ( 'new_color_price_old_' . $item );
	...
	function update_record_color($record_id,$tablename){
		
		$row2 = array();
		$price_color = $this-> get_records('record_id = '.$record_id,'fs_products_price','*');

		if(!empty($price_color)) {
			$str_color = ',';
			foreach ($price_color as $price) {
				$color = $this-> get_record('id = '.$price-> color_id,'fs_products_colors');
				if(!empty($color)){
					$str_color .= $color-> id.',';
				}
		...
		$row2['color'] = $str_color;
		$this->_update ( $row2, 'fs_products', 'id = '.$record_id );
		$this->_update ( $row2, $tablename, 'record_id = '.$record_id );

		return true;

	}
		
function remove_color($record_id,$tablename) {
	...
	function save_exist_regions($record_id) {
		...
		foreach ($other_regions_exit as $item){
			...
			$row ['is_default'] = FSInput::get ( 'regions_default_exist_' . $item );
	...
	function save_new_regions($record_id) {
		...
		foreach($other_regions as $item){
			...
			$row ['is_default'] = FSInput::get ( 'news_regions_default_' . $item );
	...
	function save_exist_utilities($record_id) {
		$rs = 0;	
		global $db;
		$other_species_exit= FSInput::get('other_utilities_exit',array(),'array');	
		foreach ($other_species_exit as $item){
			$id_exist = FSInput::get ( 'id_utilities_exist_' . $item );
			$row ['record_id'] = $record_id;
			$species = $this->get_record_by_id ( $item, 'fs_products_utilities' );
			$row ['utilities_id'] = $item;
			$row ['utilities_name'] =$species->name;
			$row ['price'] = FSInput::get ( 'utilities_price_exist_' . $item );
			$u = $this->_update ( $row, 'fs_utilities_price', ' id=' . $id_exist );
			if ($u)
				$rs ++;
	...
	function save_exist_range_times($record_id) {
		$rs = 0;	
		global $db;
		$other_species_exit= FSInput::get('other_range_times_exit',array(),'array');	
		foreach ($other_species_exit as $item){
			$id_exist = FSInput::get ( 'id_range_times_exist_' . $item );
			$row ['record_id'] = $record_id;
			$species = $this->get_record_by_id ( $item, 'fs_products_soccer_range_times' );
			$row ['range_times_id'] = $item;
			$row ['range_times_name'] = $species->name;
			$row ['earliest_time'] = $species->earliest_time;
			$row ['latest_time'] = $species->latest_time;
			$row ['earliest_time_int'] = $species->earliest_time_int;
			$row ['latest_time_int'] = $species->latest_time_int;
			$row ['price'] = FSInput::get ( 'range_times_price_exist_' . $item );
			$row ['price_t7'] = FSInput::get ( 'range_times_price_t7_exist_' . $item );
			$row ['price_cn'] = FSInput::get ( 'range_times_price_cn_exist_' . $item );
			$u = $this->_update ( $row, 'fs_products_soccer_range_times_price', ' id=' . $id_exist );
			if ($u)
				$rs ++;
	...
	function save_new_range_times($record_id) {
		if (! $record_id)
			return true;
		$other_species = FSInput::get('other_range_times',array(),'array');	
		global $db;
		foreach($other_species as $item){
			...
			$row ['earliest_time_int'] = $species->earliest_time_int;
			$row ['latest_time_int'] = $species->latest_time_int;
			$row ['price'] = FSInput::get ( 'new_range_times_price_' . $item );
			$row ['price_t7'] = FSInput::get ( 'new_range_times_price_t7_' . $item );
			$row ['price_cn'] = FSInput::get ( 'new_range_times_price_cn_' . $item );
			$rs = $this->_add ( $row, 'fs_products_soccer_range_times_price', 0 );
		}
		return true;
	}
	...
	function save_new_utilities($record_id) {
		if (! $record_id)
			return true;
		$other_species = FSInput::get('other_utilities',array(),'array');	
		global $db;
		foreach($other_species as $item){
			$row = array ();
			$row ['record_id'] = $record_id;
			...
			$row ['utilities_id'] = $item;
			$row ['utilities_name'] =$species->name;
			$row ['price'] = FSInput::get ( 'new_utilities_price_' . $item );
			$rs = $this->_add ( $row, 'fs_utilities_price', 0 );
		}
		return true;
	}
	...
	function remove_utilities($record_id) {
		$rs = 0;	
		global $db;
		$other_species_exit= FSInput::get('other_utilities_exit',array(),'array');	
		$str_other_species = implode ( ',', $other_species_exit );
		$whewe ='';
		if ($str_other_species) {
			$whewe .=' AND utilities_id NOT IN ('.$str_other_species.')';
		}
		$sql = " DELETE FROM fs_utilities_price
		WHERE record_id = ".$record_id."".
		$whewe;
		$db->query ( $sql );
		$rows = $db->affected_rows ();
	...
	function remove_range_times($record_id) {
		$rs = 0;	
		global $db;
		$other_species_exit= FSInput::get('other_range_times_exit',array(),'array');	
		$str_other_species = implode ( ',', $other_species_exit );
		$whewe ='';
		if ($str_other_species) {
			$whewe .=' AND range_times_id NOT IN ('.$str_other_species.')';
		}
		$sql = " DELETE FROM fs_products_soccer_range_times_price
		WHERE record_id = ".$record_id."".
		$whewe;
		$db->query ( $sql );
		$rows = $db->affected_rows ();
	...
	function save_exist_warranty($record_id) {
		...
		foreach ($other_warranty_exit as $item){
			...
			$row ['is_default'] = FSInput::get ( 'warranty_default_exist_' . $item );
	...
	function save_new_warranty($record_id) {
		...
		foreach($other_warranty as $item){
			...
			$row ['is_default'] = FSInput::get ( 'new_warranty_default_' . $item );
	...
	// 					FROM fs_products_soccer_images_plus
	// 					WHERE record_id = $record_id";
	// 	global $db;
	// 	$sql = $db->query ( $query );
	// 	$result = $db->getObjectList ();
	...
	// 			}
	// 		// remove in database
	// 		$sql = " DELETE FROM fs_products_soccer_images_plus
	// 					WHERE record_id = ".$record_id."".
	// 						$whewe;
	// 		$db->query ( $sql );
	// 		$rows = $db->affected_rows ();
	...
	// 		$u = $this->_update ( $row, 'fs_products_soccer_images_plus', ' id=' . $item );
	// 		if ($u)
	// 			$rs ++;
	...
	// 			$rs = $this->_add ( $row, 'fs_products_soccer_images_plus', 0 );
	// 		}
	// 	}
	// 	return true;
	// }	
...
function upload_other_slideshow_highlight() {
	...
	function get_data_by_warranty($warranty_id,$record_id){
		...
		function get_data_by_utilities($utilities_id,$record_id){
			global $db;
			if(!$utilities_id)
				return false;
			
			$query   = " SELECT *
			FROM fs_utilities_price 
			WHERE utilities_id = ".$utilities_id."
			AND record_id =".$record_id;
			$db->query($query);
			$result = $db->getObject();
			return $result;
		...
		function get_data_by_range_times($range_times_id,$record_id){
			global $db;
			if(!$range_times_id)
				return false;
			
			$query   = " SELECT *
			FROM fs_products_soccer_range_times_price 
			WHERE range_times_id = ".$range_times_id."
			AND record_id =".$record_id;
			$db->query($query);
			$result = $db->getObject();
			return $result;
		...
		function remove_cache()
		{
			...
			$link_detail = FSRoute::_('index.php?module=products_soccer&view=product&id='.$data->id.'&code='.$data -> alias.'&ccode='.$data-> category_alias);
		...
		function uploadAjaxVideoReview() {
			...
			foreach ($file_names as $file_name2 ) {
				...
				$rs = $this->_add ( $row, 'fs_products_soccer_video_review' );
			}
			return true;
		...
		function get_max_images_video_review($record_id,$session_id = ''){
			...
			FROM fs_products_soccer_video_review
			WHERE $where
			";
		...
		function deleteAjaxImagesVideoReview($record_id = 0) {
			...
			if ($listImages) {
				foreach ( $listImages as $item ) {
					$query = '  DELETE FROM fs_products_soccer_video_review
					WHERE id = \'' . $item->id . '\'';
					$db->query ( $query );
					$path = PATH_BASE . $item->image;
					@unlink ( $path );
		...
		function sortAjaxImagesVideoReview() {
			...
			if (!empty($arr)) {
				foreach ( $arr as $key => $value ) {
					$db->query ( "UPDATE fs_products_soccer_video_review SET ordering = $key WHERE id = $value" );
				}
			}
		}
		...
		function addTitleAjaxImagesVideoReview($record_id = 0) {
			...
			$rs = $this->_update ( $row, 'fs_products_soccer_video_review',$where);
			return $rs;
		}
		...
		function ajax_get_tags(){
			...
			if($tags_arr[count($tags_arr) - 1] != ''){

				$all_tags = str_replace($tags_arr[count($tags_arr) - 1],'',$tags);
				
				$get_data = $this->get_records('name like "%' .$tags_arr[count($tags_arr) - 1].'%"','fs_products_tags');

				if(!empty($get_data)){
					$html ='';
					foreach ($get_data as $item) {

						$html .= '<span onclick = "re_news_tags('."'".$all_tags.$item->name."'".')" class="item-tag">'.$item->name.'</span>';
					}
					return $html;
				}
			}
