<?php 
	class Products_soccerModelsComments extends FSModels{
...
//		}
	...
	function get_comments_by_product($record_id) {
		...
		$query = " SELECT rate,name,created_time,id,email,comment,parent_id, published,record_id
						FROM fs_products_soccer_comments
						$where
	...
	function ajax_published($published = 1) {
		...
		$rs = $this->_update ( $row, 'fs_products_soccer_comments', 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ' );
		echo $rs ? 1 : 0;
		if ($rs) {
	...
	function ajax_del() {
		...
		$rs = $this->_remove ( 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ', 'fs_products_soccer_comments' );
		echo $rs ? 1 : 0;
		if ($rs) {
			$this->recal_comments ( $record_id );
		}
	...
	function recal_comments($record_id) {
		$list = $this->get_records ( 'record_id = ' . $record_id, 'fs_products_soccer_comments', 'id,published' );
		$total_published = 0;
	...
	function update_unread_for_comments($record_id) {
		$row ['readed'] = 1;
		$this->_update ( $row, 'fs_products_soccer_comments', 'record_id = ' . $record_id );
	...
	function save_comment() {
		...
		if(strtolower($name) == 'msmobile') $name = 'MSmobile';	
		...
		$rs = $this -> _add($row, 'fs_products_soccer_comments');
		echo $rs?1:0;
		if ($rs)
			$this->recal_comments ( $record_id );
		return $rs;
...
//		$row ['created_time'] = $time;
		...
		$rs = $this -> _update($row, 'fs_products_soccer_comments',' id = '.$comment_id);
		echo $rs?1:0;
		if ($rs)
			$this->recal_comments ( $record_id );
		return $rs;
	...
	function save_point() {
		...
		$rs = $this->_update ( $row, FSTable_ad::_ ('fs_products_soccer_comments'), ' id = ' . $comment_id );
