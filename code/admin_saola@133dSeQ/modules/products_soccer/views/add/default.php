...
<fieldset id="zone-upload-analytics">
	...
	<?php }else {?>
		<legend class="title">Thêm mới sản phẩm từ excel</legend>
<!--		<span style="color:red;">Cột Name trong file exel phải trùng với tên sản phẩm</span>-->
	<?php }?>
	<div id="content-form-upload-import-excel">
		<form id="frm-import-excel-for-product" action="index.php?module=products_soccer&view=add&task=add_product" method="POST" enctype="multipart/form-data">
			...
			<div class="one-row-input">
				...
				<div class="input">
					<input type="submit" value="<?php echo (@$cat)?'Import':'Thêm mới'?>">
					&nbsp;
					<input type="button" value="Nhập file khác" onclick="javascritp: location.reload();">
					<?php if(@$cat){?>
					<input type="button" value="Mẫu import" onclick="location.href='index.php?module=products_soccer&view=add&task=extract_file&cid=<?php echo $cat->id;?>&raw=1' ">
					<?php }else{?>
						<input type="button" value="Mẫu  import" onclick="location.href='index.php?module=products_soccer&view=add&task=download_file&raw=1' ">
					<?php }?>
					<input type="button" value="Hướng dẫn nhập" onclick="location.href='index.php?module=products_soccer&view=add&task=download_file_hd&raw=1' ">
				</div>
			</div>
