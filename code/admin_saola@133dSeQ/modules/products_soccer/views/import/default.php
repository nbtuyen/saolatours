...
<!--		<span style="color:red;">Cột Name trong file exel phải trùng với tên sản phẩm</span>-->
	...
	<div id="content-form-upload-import-excel">
		<form id="frm-import-excel-for-product" action="index.php?module=products_soccer&view=import&task=import_product" method="POST" enctype="multipart/form-data">
			...
			<div class="one-row-input">
				...
				<div class="input">
					...
					<?php if(@$cat){?>
					<input type="button" value="Mẫu  import" onclick="location.href='index.php?module=products_soccer&view=import&task=extract_file&cid=<?php echo $cat->id;?>&raw=1' ">
					<?php }else{?>
						<input type="button" value="Mẫu  import" onclick="location.href='index.php?module=products_soccer&view=import&task=download_file&raw=1' ">
					<?php }?>
				</div>
			</div>
