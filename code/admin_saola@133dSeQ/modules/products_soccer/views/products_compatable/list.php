				...
				<tbody>
					...
					<?php if(@$list){?>
						<?php foreach ($list as $row) { ?>
							<?php $link_detail = "index.php?module=products_soccer&view=products&task=edit&id=".$row->id; ?>
...
<script type="text/javascript">
	...
	function add_product(product_compatable_id){
		$.get("index.php?module=products_soccer&view=products_compatable&task=add_products&raw=1",{product_compatable_id: product_compatable_id,id:id}, function(j){
			 j = eval("(" + j + ")");
			status = j['status'];
			if(status == 0){
				alert('Không add thành công');
			}else if(status == 1){
				html = j['html'];
				alert('Đã thêm thành công');
				row_continue = window.opener.$(".record_compatable_continue");
