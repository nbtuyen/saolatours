<?php  
	global $toolbar;
	$toolbar->setTitle(FSText :: _('Đánh giá') );
//
//	//	CONFIG	
//	$list_config = array();
...
//	FILTER
	...
	$text_record_id['title'] =  'Id sản phẩm';
								...
								<tbody>
									...
									<?php if($count){?>
										<?php foreach ($list as $row) { ?>
											<?php $link_detail = FSRoute::_('index.php?module=products_soccer&view=product&id='.$row->id.'&code='.$row -> alias.'&ccode='.$row-> category_alias); ?>
											<tr class="row<?php echo $i%2; ?> product_record" id="record_<?php echo $row -> id; ?>">
												<td align="left">
													<span class='title'><?php echo $row->name; ?></span>
													<br/>
													<div class='product_rates'>
														<span><strong class='unread' id='unread_<?php echo $row->id; ?>'><?php echo $row -> rates_unread; ?></strong> chưa đọc</span>
														<span><strong class='published' id='published_<?php echo $row->id; ?>'><?php echo $row -> rates_published; ?></strong> hiển thị</span>
														<span><strong class='unpublished' id='unpublished_<?php echo $row->id; ?>'><?php echo ($row -> rates_total - $row -> rates_published); ?></strong> ẩn</span>
					...
					<td width="70%">
						<table border="1" class="tbl_form_contents">
							<thead>
								<th class="title" >
									<td width="70%">
										<div class='product_title' id='product_title'></div>
									</td>
									<td width="30%">
...
//		$( "#text1" ).datepicker({clickInput:true,dateFormat: 'dd-mm-yy'});
	...
	function call_rates(){
		$('.product_record').click(function(){
			id = $(this).attr('id');
			record_id = id.replace('record_','');
			ajax_call_rates(record_id);
//			$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id}, function(html){
//				$('#rates_area').html(html);
//				$('#unread_'+record_id).html(0);
//				redisplay_rates_static_on_tabhead(record_id);
//			});
			$('.product_record').removeClass('activated');
			$(this).addClass('activated');
		});
	}
	function ajax_call_rates(record_id){
		type = $('#type_comment').val();
		$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id,type:type}, function(html){
			$('#rates_area').html(html);
			$('#unread_'+record_id).html(0);
			redisplay_rates_static_on_tabhead(record_id);
		});
	}
	function call_rates_by_type(){
	...
	function ajax_unpublished(id,record_id){
		$.get("index2.php?module=products_soccer&view=rates&task=ajax_unpublished&raw=1",{id:id,record_id:record_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id}, function(html){
	...
	*/
	function redisplay_rates_static_on_tabhead(record_id){
		no_published = $('#published_'+record_id).html();
		no_unpublished = $('#unpublished_'+record_id).html();
	...
	function ajax_published(id,record_id){
		$.get("index2.php?module=products_soccer&view=rates&task=ajax_published&raw=1",{id:id,record_id:record_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id}, function(html){
	...
	function ajax_del(id,record_id,published){
		if(confirm('Bạn chắc chắn muốn xóa?')){
			$.get("index2.php?module=products_soccer&view=rates&task=ajax_del&raw=1",{id:id,record_id:record_id}, function(status){
				if(status == 1){
					type = $('#type_comment').val();
					$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id,type:type}, function(html){
//					$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id}, function(html){
	...
	function submit_reply(parent_id,record_id){
		...
		$.get("index.php?module=products_soccer&view=rates&task=ajax_save_comment&raw=1",{record_id:record_id,parent_id:parent_id,name: name,text:reply_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=product&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id}, function(html){
					$('#rates_area').html(html);
				});
				no_published = $('#published_'+record_id).html();
				no_published ++ ;
	...
	function submit_edit(comment_id,record_id){
		...
		$.get("index.php?module=products_soccer&view=rates&task=ajax_edit_comment&raw=1",{record_id:record_id,comment_id:comment_id,name: name,text:edit_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products_soccer&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=product&view=rates&task=ajax_get_rates_by_product&raw=1",{record_id:record_id}, function(html){
...
.product_list .activated a,.product_list .activated span,.product_list .activated{
	color: red;
	font-weight: bold;
}
#product_title {
