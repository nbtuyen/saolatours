<?php
class ProductsControllersAdd  extends Controllers
{
	...
	function display()
	{
		...
		include 'modules/'.$this->module.'/views/add/default.php';
	}
	...
	function upload_excel($input_name)
	{
		...
		if(	!$excel){
			...
			if($cid){
				...
				if($rs)
				{
					setRedirect("index.php?module=products_soccer&view=add&cid=$cid",FSText::_('Có').'<strong> '.$rs.'</strong> '.'bản ghi được thêm vào','suc');
				}
				else 
				{
					setRedirect("index.php?module=products_soccer&view=add&cid=$cid",FSText::_('Không có bản ghi nào được thêm vào'),'alert');
				}	
			}else{
                                        // echo $path."ssss";die;
				...
				if($rs)
				{
					setRedirect("index.php?module=products_soccer&view=add",FSText::_('Có').'<strong> '.$rs.'</strong> '.'bản ghi được thêm vào','suc');
				}
				else 
				{

					setRedirect("index.php?module=products_soccer&view=add",FSText::_('Không có bản ghi nào được thêm vào'),'alert');
				}	
			}
...
//			}
	...
	function download_file(){
		...
		$file_export_name = 'Mau_them_san_pham';
		$file_ext = $this -> getExt(basename('mau_import'.DS.'Mau_them_san_pham.xls'));
		...
		function download_file_hd(){
		
		$path_file = PATH_BASE.DS.'mau_import'.DS.'Mau_san_pham.xls'; 
		$fsstring = FSFactory::getClass('FSString');
		$file_export_name = 'Huong_dan_them_san_pham';
		$file_ext = $this -> getExt(basename('mau_import'.DS.'Huong_dan_them_san_pham.xls'));
		$file_export_name = $file_export_name.'.'.$file_ext;
		header("Pragma: public");
		header("Expires: 0");
		header("Cache-Control: no-cache,must-revalidate, post-check=0, pre-check=0");
