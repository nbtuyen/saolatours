<?php
class ProductsControllersCombo  extends Controllers
{
			...
			if(isset($_REQUEST['keysearch']) AND !empty($_REQUEST['keysearch']))
			{
				...
				if(!empty($get_menu)){
					foreach ($get_menu as $key => $item) {
						$link = 'index.php?module=products_soccer&view=combos&task=add&cid='.$item->id;
		...
		function get_menu_tree($parent_id) 
		{
			...
			if(!empty($list)){
				foreach ($list as $key => $item) {
					$link = 'index.php?module=products_soccer&view=combo&task=add&cid='.$item->id;
		...
		//$excel->obj_php_excel->getActiveSheet ()->setCellValue ( 'I10', 'Thành tiền (VNĐ)' );
			...
			//	die();
				...
				foreach ($list as $item){
					...
					$link = FSRoute::_('index.php?module=products_soccer&view=combo&code='.$item -> alias.'&ccode='.$item->category_alias.'&id='.$item->id.'&cid='.$item->category_id.'&Itemid=35');
...
function view_name($controle,$id){
	...
	$link = FSRoute::_('index.php?module=products_soccer&view=combo&id='.$data->id.'&code='.$data -> alias.'&ccode='.$data-> category_alias);
	if($data->published != 1){
		return 'Chưa kích hoạt !';
	}else{
		return '<a target="_blink" href="' . $link . '" title="Xem ngoài font-end">Xem ngoài font-end</a>';
	}
