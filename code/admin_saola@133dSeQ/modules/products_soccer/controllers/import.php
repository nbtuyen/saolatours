	...
	function upload_excel($input_name)
	{
		...
		if(	!$excel){
			...
			if($cid){
				...
				if($rs)
				{
					setRedirect("index.php?module=products_soccer&view=import&cid=$cid",FSText::_('Có').'<strong> '.$rs.'</strong> '.'bản ghi được cập nhật','suc');
				}
				else 
				{
					echo "1";
					die(1);
					setRedirect("index.php?module=products_soccer&view=import&cid=$cid",FSText::_('Không có bản ghi nào được cập nhật'),'alert');
				}	
			}else{
                                        // echo $path."ssss";die;
				...
				if($rs)
				{
					setRedirect("index.php?module=products_soccer&view=import",FSText::_('Có').'<strong> '.$rs.'</strong> '.'bản ghi được cập nhật','suc');
				}
				else 
				{

					setRedirect("index.php?module=products_soccer&view=import",FSText::_('Không có bản ghi nào được cập nhật'),'alert');
				}	
			}
...
//				$excel->obj_php_excel->getActiveSheet()->duplicateStyle( $excel->obj_php_excel->getActiveSheet()->getStyle('A1'), 'B1:F1' );
		...
		echo $link_excel = URL_ROOT.'admin_adminlnn/export/excel/'. $filename.'.xls';
		?>
...
//			}
	...
	function download_file(){
		...
		echo $link_excel = URL_ROOT.'admin_adminlnn/export/excel/'. $file_export_name;
		?>
		<?php setRedirect($link_excel); ?>
		<?php 
		readfile($path_file);
