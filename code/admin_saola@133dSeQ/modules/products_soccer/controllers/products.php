<?php
class Products_soccerControllersProducts  extends Controllers
{
	...
	function display()
	{
		...
		$categories_products = $model->get_categories_products_tree();
		$type = $model->get_type();
	...
	function add()
	{
		...
		$peoples = $model -> get_records('','fs_products_peoples');
		$categories_products = $model->get_categories_products_tree();
		...
		if($cid)
		{
			...
			$utilities = $model-> get_records('published = 1','fs_products_utilities');
			$range_times = $model-> get_records('published = 1','fs_products_soccer_range_times');
				// types
			$types = $model -> get_records('published = 1','fs_products_types');
			...
			if(isset($_REQUEST['keysearch']) AND !empty($_REQUEST['keysearch']))
			{
				...
				if(!empty($get_menu)){
					foreach ($get_menu as $key => $item) {
						$link = 'index.php?module=products_soccer&view=products&task=add&cid='.$item->id;
		...
		function get_menu_tree($parent_id) 
		{
			...
			$model = $this -> model;
			$menu = "";
			$list = $model->get_records('published = 1 AND parent_id =' . $parent_id, 'fs_products_soccer_categories','*','name ASC ');
			if(!empty($list)){
		...
		function edit()
		{
			...
			$list_image_summary = $model -> get_records('record_id = '.$data -> id,'fs_products_soccer_image_summary');
			$categories_products = $model->get_categories_products_tree();
			...
			$products_soccer_related = $model -> get_products_soccer_related($data -> products_soccer_related);
...
//			$images_plus = $model->get_product_images_plus($data -> id);
			...
			foreach (@$utilities as $item){
				$data_by_utilities = $model -> get_data_by_utilities($item->id,$data->id );
				if(!empty($data_by_utilities)){
					$array_data_by_utilities [$item->id] = $data_by_utilities;	
				}
			}
			...
			foreach (@$range_times as $item){
				$data_by_range_times = $model -> get_data_by_range_times($item->id,$data->id );
				if(!empty($data_by_range_times)){
					$array_data_by_range_times[$item->id] = $data_by_range_times;	
				}
			}
			...
			if($data->city_id){
				$districts  = $model -> get_records('city_id = '. $data->city_id,'fs_districts','id,name,alias');
			}
...
//				$excel->obj_php_excel->getActiveSheet()->getStyle('A1')->getAlignment()->setIndent(1);// padding cell
		...
		//sân bóng phẩm liên quan
		function ajax_get_products_soccer_related(){
			$model = $this -> model;
			$data = $model->ajax_get_products_soccer_related();
			$html = $this -> products_soccer_genarate_related($data);
			echo $html;
			return;
		}
		function products_soccer_genarate_related($data){
			$str_exist = FSInput::get('str_exist');
			$html = '';
			$html .= '<div class="products_soccer_related">';
			foreach ($data as $item){
				if($str_exist && strpos(','.$str_exist.',', ','.$item->id.',') !== false ){
					$html .= '<div class="red products_soccer_related_item  products_soccer_related_item_'.$item -> id.'" onclick="javascript: set_products_soccer_related('.$item->id.')" style="display:none" >';	
					$html .= $item -> name;				
					$html .= '</div>';					
				}else{
					$html .= '<div class="products_soccer_related_item  products_soccer_related_item_'.$item -> id.'" onclick="javascript: set_products_soccer_related('.$item->id.')">';	
					$html .= $item -> name;				
					$html .= '</div>';	
				}
			}
		...
		function ajax_getExtendFields(){
			...
			$data_cid_before = $model->get_record('id = '. $cid_before ,'fs_products_soccer_categories','id, tablename');

			$data_cid_after = $model->get_record('id = '. $cid_after ,'fs_products_soccer_categories','id, tablename');
...
//	} 
	...
	function ajax_get_districts(){
		$model  = $this -> model;
		$sl_city = FSInput::get('sl_city');
		$rs  = $model -> get_records('city_id = '. $sl_city,'fs_districts','id,name,alias');
		$data = array ('error' => true, 'message' => '', 'html' => '<option value="">Chọn Quận/Huyện</option>' );
		if($rs){
			foreach ( $rs as $item ) {
				$data ['html'] .= '<option data_id = "'.$item->id.'" value="' . $item->id . '">' . $item->name . '</option>';
			}
		}
		$data ['error'] = false;
...
function view_name($controle,$id){
	...
	$data = $model->get_record('id = ' .$id,'fs_products_soccer','id,alias,category_alias,published');
...
function change_time_detail($controle,$id) {
	$link = 'index.php?module=products_soccer&view=soccer_time&task=add&products_soccer_id='.$id.'&page=1';
	return '<a href="'.$link.'" target="_blink"><svg width="16px" height="16px" fill="#3DA6EA" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="search-plus" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-search-plus fa-w-16 fa-2x"><path  d="M304 192v32c0 6.6-5.4 12-12 12h-56v56c0 6.6-5.4 12-12 12h-32c-6.6 0-12-5.4-12-12v-56h-56c-6.6 0-12-5.4-12-12v-32c0-6.6 5.4-12 12-12h56v-56c0-6.6 5.4-12 12-12h32c6.6 0 12 5.4 12 12v56h56c6.6 0 12 5.4 12 12zm201 284.7L476.7 505c-9.4 9.4-24.6 9.4-33.9 0L343 405.3c-4.5-4.5-7-10.6-7-17V372c-35.3 27.6-79.7 44-128 44C93.1 416 0 322.9 0 208S93.1 0 208 0s208 93.1 208 208c0 48.3-16.4 92.7-44 128h16.3c6.4 0 12.5 2.5 17 7l99.7 99.7c9.3 9.4 9.3 24.6 0 34zM344 208c0-75.2-60.8-136-136-136S72 132.8 72 208s60.8 136 136 136 136-60.8 136-136z" class=""></path></svg></a>'; 
}
