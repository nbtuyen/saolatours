<?php
	class ProductsControllersAdd_update_excel  extends Controllers
	{
		...
		function display()
		{
			...
			if($cid)
			{
				$cat =  $model->get_record('id='.$cid,'fs_products_soccer_categories');
			}
			
			include 'modules/'.$this->module.'/views/add_update_excel/default.php';
		}
		...
		function upload_excel($input_name)
		{
			...
			if(	!$excel){
				...
				if($cid){
					...
					if($rs)
					{
						setRedirect("index.php?module=products_soccer&view=add_update_excel&cid=$cid",FSText::_('Có').'<strong> '.$rs.'</strong> '.'bản ghi được cập nhật','suc');
					}
					else 
					{
						setRedirect("index.php?module=products_soccer&view=add_update_excel&cid=$cid",FSText::_('Không có bản ghi nào được cập nhật'),'alert');
					}	
				}else{
                                        // echo $path."ssss";die;
					...
					if($rs)
					{
						setRedirect("index.php?module=products_soccer&view=add_update_excel",$rs,'suc');
					}
					else 
					{
						setRedirect("index.php?module=products_soccer&view=add_update_excel",FSText::_('Không có bản ghi nào được cập nhật'),'alert');
					}	
				}
		...
		function extract_file(){
			...
			$catagories =  $model->get_record_by_id($cat_id,'fs_products_soccer_categories');
...
//			}
		...
		// 	$path_file = PATH_BASE.'mau_import'.DS.'them_san_pham.xls'; 
		
		// 	$fsstring = FSFactory::getClass('FSString');
		// 	$file_export_name = 'them_san_pham';
		// 	$file_ext = $this -> getExt(basename('mau_import'.DS.'them_san_pham.xls'));
		...
		function download_file(){
			...
			$path_file = PATH_BASE.LINK_AMIN.'export'.DS.'excel'.'them_san_pham.xls'; 
			$fsstring = FSFactory::getClass('FSString');
			$file_export_name = 'them_san_pham';
			$file_ext = $this -> getExt(basename('export'.DS.'excel'.DS.'them_san_pham.xls'));
			$file_export_name = $file_export_name.'.'.$file_ext;
			header("Pragma: public");
			header("Expires: 0");
