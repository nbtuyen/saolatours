<?php
	class NewsControllersComments extends Controllers{
		...
		function ajax_get_comments_by_news()
		{
			...
			$model = $this -> model;
			$data = $model->get_comments_by_news($record_id);
		...
		function ajax_get_comments_by_project()
		{
			$record_id = FSInput::get('record_id',0,'int');
			...
			$model = $this -> model;
			$data = $model->get_comments_by_product($record_id);
			$html = $this -> genarate_comments($data);
			echo $html;
			if(count($data)){
				$model->update_unread_for_comments($record_id);	
			}
			// //gửi email cho ng chủ comment bài viết
			// $data_admin_reply = $model->get_comments_by_admin_reply($parent_id);
			...
			// $mailer -> isHTML(true);
			// $mailer -> setSender(array('buixuanthangcntt2@gmail.com','Quản trị viên'));
			// $mailer -> AddAddress($data_admin_reply->email,$data_admin_reply->name);
			// $mailer -> setSubject($site_name.' - Quản trị viên đã trả lời bình luận');
			// // body
			// $body = '';
			// $body .= '<div>Chào '.$data_admin_reply->name.'!</div>';
			// $body .= '<div>Quản trị viên đã phản hồi bình luận của bạn ở bài viết: '.$data_admin_reply->link.'</div>';
...
//			$this -> arr_status = array(1=>'Lưu nháp',2=>'BTV từ chối',3=>'Chờ BTV duyệt',4=>'Đã biên tập',5=>'Hạ bài (kéo về)',6=>'Xuất bản');
		...
		function view_comment($news){
			$link = 'index.php?module=news&view=comments&keysearch=&text_count=1&text0='.$news ->id.'&filter_count=1&filter0=0';
			$html = '';
			if($news -> comments_unread){
				$html .= '<strong>'.$news -> comments_unread.'/'.$news -> comments_total.'</strong>';
			}else{
				$html .= $news -> comments_unread.'/'.$news -> comments_total;
			}
