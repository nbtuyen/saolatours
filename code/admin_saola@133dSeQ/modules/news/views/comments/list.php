								...
								<tbody>
									...
									<?php if($count){?>
										<?php foreach ($list as $row) { ?>
											...
											<tr class="row<?php echo $i%2; ?> project_record" id="record_<?php echo $row -> id; ?>">
												<td align="left">
...
//			$.get("index2.php?module=news&view=comments&task=ajax_get_comments_by_project&raw=1",{record_id:record_id}, function(html){
//				$('#comments_area').html(html);
//				$('#unread_'+record_id).html(0);
//				redisplay_comments_static_on_tabhead(record_id);
//			});
			$('.project_record').removeClass('activated');
			$(this).addClass('activated');
		});
	}
	function ajax_call_comments(record_id){
		type = $('#type_comment').val();
		$.get("index2.php?module=news&view=comments&task=ajax_get_comments_by_project&raw=1",{record_id:record_id,type:type}, function(html){
			$('#comments_area').html(html);
			$('#unread_'+record_id).html(0);
			redisplay_comments_static_on_tabhead(record_id);
		});
...
//					$.get("index2.php?module=news&view=comments&task=ajax_get_comments_by_project&raw=1",{record_id:record_id}, function(html){
	...
	function submit_reply(parent_id,record_id){
		...
		$.get("index.php?module=news&view=comments&task=ajax_save_comment&raw=1",{record_id:record_id,parent_id:parent_id,name: name,text:reply_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=news&view=comments&task=ajax_get_comments_by_project&raw=1",{record_id:record_id,type:type,parent_id:parent_id}, function(html){
//				$.get("index2.php?module=project&view=comments&task=ajax_get_comments_by_project&raw=1",{record_id:record_id}, function(html){
					$('#comments_area').html(html);
				});
				no_published = $('#published_'+record_id).html();
				no_published ++ ;
...
.project_list .activated a,.project_list .activated span,.project_list .activated{
	color: red;
	font-weight: bold;
}
#project_title {
