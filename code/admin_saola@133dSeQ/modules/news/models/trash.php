<?php
class NewsModelsTrash extends FSModels {
	var $limit;
	var $prefix;
	var $image_watermark;
	...
	function setQuery() {
		...
		if(isset($_SESSION[$this -> prefix.'text0']))
		{
			...
			if($date_from){
				...
				$where .= ' AND a.updated_time >=  "'.$date_new.'" ';
			}
		}
		...
		if (! $ordering)
			$ordering .= " ORDER BY updated_time DESC , id DESC ";
		
		if (isset ( $_SESSION [$this->prefix . 'keysearch'] )) {
			if ($_SESSION [$this->prefix . 'keysearch']) {
				$keysearch = $_SESSION [$this->prefix . 'keysearch'];
				$where .= " AND ( a.title LIKE '%" . $keysearch . "%' OR a.alias LIKE '%" . $keysearch . "%' OR a.id = '".$keysearch."')";
			}
		}
