...
class Remote_db {
    ...
    function connect() {
    	...
    	$serverName = "DESKTOP-7VU1TDE\SQLEXPRESS"; 
		$uid = "sa";   
		$pwd = '123456';
		$databaseName = "suachua24hnew";
		// $databaseName = "suachualaptop";
		...
		$this->conn_id = sqlsrv_connect( $serverName, $connectionInfo);  
	...
	function query($query_string) {
		...
		// die;
		$this->query_id = @sqlsrv_query($this->conn_id,$query_string);
		if (!$this->query_id){
...
//			return false;
	...
	function getObjectList($query_id=-1) {
		$data=array();
		if ($query_id!=-1) $this->query_id=$query_id;
		while($row=@sqlsrv_fetch_object($this->query_id))
		{
			$data[]=$row;
		}
		$this->close();
    ...
    function getObjectListByKey($key,$query_id=-1){
    	...
    	if(!$key)
    		$key = 'id';
		if ($query_id!=-1) $this->query_id=$query_id;
		while($row=@sqlsrv_fetch_object($this->query_id, SQLSRV_FETCH_NUMERIC))
		{
			$data[$row->$key]=$row;
		}
		return $data;
    ...
    function getObject($query_id=-1)
    {
		...
		$row = sqlsrv_fetch_object( $this->query_id, SQLSRV_FETCH_NUMERIC)  ;
		return $row;
