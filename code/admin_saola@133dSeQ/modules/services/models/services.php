<?php 
	class ServicesModelsServices extends FSModels
	{
		...
		function __construct()
		{
			...
			$this -> table_category_name = FSTable_ad::_ ('fs_services_categories');
			$this -> table_types = FSTable_ad::_ ('fs_services_types');
			$this -> arr_img_paths = array(array('resized',450,300,'resized_not_crop'),array('small',80,60,'resized_not_crop'),array('large',600,400,'resize_image'));
			$this -> table_name = FSTable_ad::_ ('fs_services');
		...
		function save($row = array(),$use_mysql_real_escape_string = 0){
			...
			$cat =  $this->get_record_by_id($category_id,FSTable_ad::_ ('fs_services_categories'));
			...
			//$row['content'] = FSInput::get('content','','txt');
	        	...
	        	foreach($field_change_arr as $field_item){
	        	      ...
	        	      if($field_value_original != $field_value_new){
	        	          ...
	        	          if($field_item == 'category_id'){
	        	          		$cat =  $this->get_record_by_id($field_value_new,FSTable_ad::_ ('fs_services_categories'));
								$row['category_id_wrapper'] = $cat -> list_parents;
								$row['category_alias_wrapper'] = $cat -> alias_wrapper;
								$row['category_name'] = $cat -> name;
								$row['category_alias'] = $cat -> alias;
